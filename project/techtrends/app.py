...
def post(post_id):
    ...
    if post is None:
      app.logger.error('A non existing article is accessed and 404 is returned')
...
# for count of posts the COUNT command will be used
@app.route('/metrics')
def metrics():
    connection = get_db_connection()
    posts = connection.execute('SELECT * FROM posts').fetchall()
    connection.close()
    response = app.response_class(
        response=json.dumps({ "db_connection_count": connections, "post_count": len(posts) }),
        status=200,
        mimetype='application/json'
    )
